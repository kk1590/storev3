...
public class TbUserController {
    ...
    public List<TbUser> findAll(){
        return tbUserService.queryAllByLimit();
    }
    ...
    public PageModel<TbUser> getTbUserByPage(int page,int size){
        return tbUserService.queryAllByLimit(page,size);
    }
    ...
    public Result deleteByIds(Long[] ids){
        try {
            tbUserService.deleteByPrimaryKeys(ids);
     ...
     * 分页查询按条件查询数据
     *
     * @param tbUser 实例对象(封装了查询的关键字)
     * @return 列表数据
     */
    @RequestMapping("search")
    public PageModel<TbUser> getTbUserByPage(@RequestBody TbUser tbUser,int page,int size){
        return tbUserService.queryAllByLimit(tbUser,page,size);
    }
